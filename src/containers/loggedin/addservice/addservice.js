...
class AddServicePage extends Component {
        ...
        if (Object.keys(nextProps.geo).length) {
            ...
            if (nextProps.profile.product) {
                ...
                if (addonPaid.length > 0) {
                    ...
                    tempPaidAddon.addon = [];
                    addonPaid.filter(addon => typeof addon.id === 'number' && addon.price !== 0).map(addon => tempPaidAddon.addon.push(addon.id));
    ...
    resetAddon() {
        this.setState({ addonData: JSON.parse(JSON.stringify(this.state.addonData)) });
    }
    ...
    setSelectedExperience(selected) {
        this.setState({ selExperience: selected === '' ? {} : this.state.packageData.service.experience[selected] });
    }
    setSelectedPackage(selected) {
        this.setState({ totalPrice: 0, selPackage: this.state.packageData.service.product[selected] });
        this.resetAddon();
    }
    moveNext() {
    ...
    render() {
        if (!Object.keys(this.props.geo).length || !Object.keys(this.props.packages).length || !Object.keys(this.state.packageData).length) {
            return (
                            ...
                            <div className="row">
                                <div className="col-lg-12 " >
                                    ...
                                    <div className="col-lg-9 " >
                                        <select className="form-control" value={this.state.selExperience.code} disabled onChange={(e) => { this.setSelectedExperience(e.target.value); }} >
                                            <option value="" >Select Experience Level</option>
                                            {Object.keys(this.state.packageData.service.experience).map(dat =>
                                                <option key={dat} value={dat} >{this.state.packageData.service.experience[dat].name} ({this.state.packageData.service.experience[dat].description})</option>)}
                                        </select>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <div className="row">
                                        <div className="col-lg-12 " >
                                            ...
                                            <Table striped bordered condensed>
                                                <tbody>
                                                    {Object.keys(this.state.packageData.service.product).filter(item => item === this.state.selPackage.code).map(item =>
                                                        (
                                                            <tr key={item}>
                                                                <td>
                                                                    <div className="col-lg-9 col-md-9 col-sm-9 col-xs-12">
                                                                        <p className="small">
                                                                            <input type="radio" name="service" disabled={false} defaultChecked={this.state.selPackage.code === item} onClick={() => { this.setSelectedPackage(item); }} value={this.state.packageData.service.product[item].code} />
                                                                            &nbsp;{this.state.packageData.service.product[item].name} {this.state.packageData.service.product[item].description ? ReactHtmlParser(`<small><i>(${this.state.packageData.service.product[item].description})</i></small>`) : ''}
                                            ...
                                            {!!Object.keys(this.state.addonData).length && this.state.addonData.filter(dat => dat.packages.indexOf(this.state.selPackage.code) !== -1).map((dat, i) =>
                                                (
                                                    <Table striped bordered condensed key={dat.title} >
                                                        <thead><tr><td><h4><i className={`${dat.icon} fa-lg`} /> {ReactHtmlParser(dat.title)}</h4></td></tr></thead>
                                                        <tbody>
                                                            {!!dat.type &&
                                                        <tr>
                                                            <td>
                                    ...
                                    <div className="row">
                                        <div className="col-lg-12">
                                            <div className="col-lg-4 col-md-4 col-md-offset-4 col-sm-3 col-xs-12">
                                                <button style={{ width: '100%' }} className="btn btn-danger btn-lg" onClick={() => { this.moveNext(); }}>Next <i className="fas fa-arrow-circle-right" aria-hidden="true" /></button> <br /><br />
                                            </div>
                                        </div>
