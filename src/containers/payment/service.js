...
class PaymentServicePage extends Component {
  ...
  componentDidMount() {
    if (this.sessionPackage) {
      ...
      if (this.sessionPackage.discount) {
        ...
        const mailinglist = Util.getDataFromLocalStorage(
          "mailinglist",
          "string"
        );
        if (mailinglist) {
          paymentApi.checkforDiscount(mailinglist).then((response) => {
            if (parseInt(response.data.discount, 10) > 0) {
              ...
              this.setState({
                ...
                totalPrice:
                  ...
                  Util.round(this.state.totalPrice * (discount / 100)),
              });
            }
          });
        }
  ...
  initialize() {
    return {
      ...
      selPackage: this.props.packageData.service.product[
        this.sessionPackage.base
      ],
      selExperience: this.props.packageData.service.experience[
        this.sessionPackage.experience
      ],
      addonData: [],
      totalPrice: 0,
      discount: 0,
  ...
  setUpPaymentForDirectAccess() {
    if (this.props.match.params.package) {
      ...
      if (
        getPackage &&
        (!getPackage.base || !getPackage.experience || !getPackage.addon)
      ) {
        this.props.history.push("/payment/servicemenu/RRS/FGR");
      }
  ...
  calTotalPrice(packageData) {
    let totalPrice = this.props.packages.product[packageData.base][
  ...
  validateForm() {
    this.setState({
      formValid:
        ...
        this.state.expMonthValid &&
        this.state.expYearValid &&
  ...
  handerPaypalSuccess = (dat) => {
    ...
    let pakdesc = `${this.state.selPackage.name} (${
    ...
    addon.push({
      ...
      price: this.props.packages.product[this.state.selPackage.code][
        this.state.selExperience.code
      ],
    });
    ...
    paymentApi.makePaypalPayment(request).then((result) => {
      this.setState({
        loader: false,
      });
      if (result.status === "Success") {
        ...
        this.props.history.push(
          `/payment/confirmation/${this.sessionPackage.base}`
        );
      } else {
  ...
  handleRequest = (e) => {
    ...
    if (paymentDone) {
      if (paymentDone === "initalize") {
        this.setState({
          loader: false,
          errorMessage: "Payment is in progress...",
        });
      } else {
    ...
    paymentApi
      ...
      .then((token) => {
        ...
        // }
        if (
          Util.getCountryData(this.props.geo.countryCode).currency !== "usd"
        ) {
          totalPrice = Util.round(totalPrice * this.state.conversionRate);
        }
        let salary = null;
        if (
          this.sessionPackage &&
          this.sessionPackage.hasOwnProperty("salary")
        ) {
          salary = SalaryData[this.sessionPackage.salary];
          salary.data = salary.data[this.props.geo.countryCode.toLowerCase()];
        }
  ...
  render() {
    return (
      <div>
        ...
        <Spinner
          message={
            'Processing your payment... <br/>Do not click multiple times on "place your order" or reload the page.'
          }
              ...
              <div className="col-md-7 col-xs-12">
                {!!this.state.discount && (
                  <div className="alert alert-success">
                    Congratualtion! you are eligible for {this.state.discount}%
                    off on resume service today
                  </div>
                )}
                      ...
                      <div id="form-container">
                        <form method="post" onSubmit={this.handleRequest}>
                          <div id="card-group" className="form-group">
                            <div className="row">
                              <div
                                className={`form-group col-xs-12 controls ${this.errorClass(
                          ...
                          <div className="row">
                            <div className="col-sm-8 col-xs-12">
                              <div className="form-group">
                                ...
                                <div
                                  id="exp_month"
                                  className={` controls ${this.errorClass(
                                    this.state.formErrors.expMonth
                                  )}`}
                                >
                                  <select
                                    className="exp-month form-control"
                                    id="expMonth"
                                    name="expMonth"
                                    onChange={this.handleUserInput}
                                  >
                                ...
                                <div
                                  id="exp_year"
                                  className={` controls ${this.errorClass(
                                    this.state.formErrors.expYear
                                  )}`}
                                >
                                  <div className="twenty">20</div>
                                  <select
                                    id="expYear"
                                    className="exp-year form-control"
                                    name="expYear"
                                    onChange={this.handleUserInput}
                                  >
                                ...
                                <FormErrors
                                  formErrors={this.state.formErrors}
                                  errorField={{ name: "expMonth" }}
                                />
                                <FormErrors
                                  formErrors={this.state.formErrors}
                                  errorField={{ name: "expYear" }}
                                />
                              </div>
                            </div>
                            <div className="col-sm-4 col-xs-12">
                              <div
                                id="cvv-group"
                                className={` form-group controls ${this.errorClass(
                              ...
                              >
                                <label htmlFor="password">
                                  ...
                                  <span
                                    ...
                                    data-tip="Creating this password will be you password to login workpapa to view the status of your project"
                                  >
                                    ?
                                  </span>
                                  <ReactTooltip />
                          ...
                          <div className="row text-center">
                            <button
                              ...
                              disabled={
                                !this.state.formValid || !!this.state.loader
                              }
                            >
                              Place Your Order
                            </button>
                          ...
                          <div className="row text-center">
                            <small>
                              By placing your order, you agree to WorkPapa's
                              Terms of Service.
                            </small>
                          </div>
                        </form>
                        ...
                        <form method="post" onSubmit={this.handlePaypalRequest}>
                          ...
                          <div className="row text-center">
                            <PaypalBtn
                              ...
                              password={this.state.password}
                              description={`${
                                this.state.selPackage.name
                              } Package`}
                              handerPaypalSuccess={this.handerPaypalSuccess}
                              handerPaypalFailer={this.handerPaypalFailer}
                            />
                            {!!this.state.loader && (
                              <img
                                alt="loader"
                ...
                <small>
                  What comes next? Good question! After you submit your order,
                  you&apos;ll receive a short questionnaire to help your writer
                  get started.
                </small>
              </div>
              <div
                className="col-md-5 col-xs-12  sidebar"
                style={{ marginTop: "73px" }}
              >
                {this.props.site.pendingResume &&
                  this.props.site.pendingResume <
                  ...
                  <table>
                    <tbody>
                      <tr>
                        ...
                        <td
                          className="price_cell text-center"
                          style={{ paddingBottom: "2px" }}
                        >
                      ...
                      <tr>
                        <td style={{ padding: "0px" }}>
                          <small>
                            Work with a writer that understands the local market
                          </small>
                        </td>
                        <td
                          className="price_cell text-right"
                          style={{ padding: "0px" }}
                        >
                          Included
                        </td>
                        ...
                        <tr>
                          ...
                          <td className="price_cell text-success text-right">
                            <h5>
                              {Util.showPrice(
                                this.state.totalPrice *
                                  (parseInt(this.state.discount, 10) / 100),
                                this.state.conversionRate,
                                this.state.currency,
                                this.state.currencyCode
                              )}
                ...
                <div className="card promiseCard">
                  ...
                  <p>
                    The first step to getting hired is getting your resume in
                    professional shape. When you work with our certified resume
                    writers, you&apos;re working with experts in <em>your</em>{" "}
                    industry.
                  </p>
                  <span
                    className="badge-guarantee guarantee-satisfaction relativeGuarantee"
                    style={{
                      background: `0 0,url(${guaranteeImg}) top center no-repeat`,
                    }}
                  />
                </div>
                ...
                <div className="card promiseCard">
                  <div
                    style={{ float: "left", margin: "10px", minHeight: "70px" }}
                  >
                  ...
                  <div>
                    ...
                    <p>
                      Your details are encrypted and protected via a 256-bit SSL
                      connection
                    </p>
                  </div>
                </div>
