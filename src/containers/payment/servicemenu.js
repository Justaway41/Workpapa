...
class PaymentServiceMenuPage extends Component {
    ...
    moveNext() {
        ...
        const packageSelected = {
            ...
            totalPrice: parseInt(this.props.packages.product[this.state.selPackage.code][this.state.selExperience.code], 10) + this.state.totalPrice,
            template,
            addon: []
        };
        // let addon = [];
                                                        ...
                                                        <tr key={item} onClick={() => { this.setSelectedPackage(item); }} >
                                                            <td style={{ cursor: 'pointer' }} className={this.state.selPackage.code === item ? 'bg-success' : ''}>
                                                                <div className="col-lg-10 col-md-10 col-sm-10 col-xs-12">
                                                                    <p className="small">
                                                                        <input type="radio" name="service" disabled={false} readOnly checked={this.state.selPackage.code === item} value={this.props.packageData.service.product[item].code} />
                                                &nbsp;{this.props.packageData.service.product[item].name} {this.props.packageData.service.product[item].description ? ReactHtmlParser(`<small><i>(${this.props.packageData.service.product[item].description})</i></small>`) : ''} - {Util.currency(this.state.currency) + Util.round(this.props.packages.product[item][this.state.selExperience.code] * this.state.conversionRate)}
                                                                        {this.props.packageData.service.product[item].code === 'RPC' &&
                                                                            <span className="text-danger"> <strong>(Most Popular)</strong></span>
                                                                        }
                                                                    </p>
                                                                </div>
                                                            ...
                                                            <div>
                                                                <div className="col-lg-9 col-md-9 col-sm-9 col-xs-12">
                                                                    <select className="form-control" onChange={(e) => { this.setSelected(dat.items, parseInt(e.target.value, 10), i); }} >
                                                                        {dat.items.map(item =>
                                                                            <option key={item.id} value={item.id} defaultValue={item.selected}>{Util.currency(this.state.currency)}{Util.round(item.price * this.state.conversionRate)} - {item.label}</option>)}
                                                                    </select>
                                                                </div>
                                    ...
                                    <div className="col-lg-12">
                                        ...
                                        <div className="col-lg-3 col-md-3 col-sm-3 col-xs-12 text-right">
                                            <h3>
                                                ...
                                                {Util.showPrice(parseInt(this.props.packages.product[this.state.selPackage.code][this.state.selExperience.code], 10) + this.state.totalPrice, this.state.conversionRate, this.state.currency, this.state.currencyCode)}
                                            </h3>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div className="panel panel-default" id="steps">
                                <div className="portfolio-item">
                                    <div className="portfolio-link">
                                        <h5 className="text-center">What&apos;s Included In Our Package?</h5>

                                        <ul style={{ listStyleType: 'none' }}>
                                            {!!Object.keys(this.props.packageData.service.product).length && Object.keys(this.props.packageData.service.product).filter(item => this.state.selPackage.code === item && this.state.selPackage.code !== 'RRS').map(item =>
                                                (
                                                    <li key={this.props.packageData.service.product[item].name}>
                                                        <small>{this.props.packageData.service.product[item].name}</small>
                                                    </li>
                                                ))}
                                            ...
                                            <li><small>A writer who knows your local job market</small></li>
                                            <li><small>Keywords that optimize your summary and get you hired</small></li>
                                            <li><small>A rewriting of your job experience that shows your true value</small></li>
                                            <li><small>An addition of numerical proof to your job experiences that make you look better to employers</small></li>
                                            <li><small>A reshuffling of your tasks within each job role to showcase your most important skills</small></li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div className="portfolio-item">
                                    <div className="portfolio-link">
                                        ...
                                        <ul style={{ listStyleType: 'none' }}>
                                            ...
                                            <li>Highly experienced resume/CV writers who know your local market and industry</li>
