...
class ProductCheckoutPage extends Component {
    constructor(props) {
        super(props);
        ...
        if (!this.sessionPackage) {
            this.props.history.push('/product/servicemenu/RRS');
        }
        this.packageData = packageData.packages[this.productType];
        this.state = this.initialize();
        this.template = {};
        this.addonData = [];
        if (addonData[this.productType]) {
            this.addonData = addonData[this.productType];
        }
    }
    componentDidMount() {
        if (this.sessionPackage) {
    ...
    initialize() {
        return {
            ...
            selPackage:
                this.packageData[this.sessionPackage.base],
            // selExperience:
    ...
    calTotalPrice(packages) {
        let totalPrice = parseInt(this.packageData[this.state.selPackage.code].price, 10);
        Object.keys(this.addonData).forEach((key) => {
            this.addonData[key].items.forEach((items) => {
                if (packages.addon.indexOf(items.id) !== -1) {
                    totalPrice += parseInt(items.price, 10);
                }
            });
        });
        // Object.keys(this.props.packages.addon).forEach((key) => {
        //     key = parseInt(key, 10);
        //     if (packages.addon.indexOf(key) !== -1) {
        //         totalPrice += this.props.packages.addon[key];
        //     }
        // });
        // totalPrice = 0.5;
                                                        ...
                                                        <div className="col-sm-6 col-xs-12">
                                                            <div className={` form-group controls ${this.errorClass(this.state.formErrors.password)}`} >
                                                                <label htmlFor="password">
                                                                    <span>Create Password</span><span className="text-danger">*</span><span className="badge badge-secondary" data-tip="Creating this password will be you password to login workpapa to view the status of your project">?</span><ReactTooltip />
                                                                </label>
                                                ...
                                                <form method="post" onSubmit={this.handlePaypalRequest} >
                                                    <div className="row" >
                                                        ...
                                                        <div className="col-sm-6 col-xs-12">
                                                            <div className={` form-group controls ${this.errorClass(this.state.formErrors.password)}`} >
                                                                <label htmlFor="password">
                                                                    <span>Create Password</span><span className="text-danger">*</span>
                                                                </label>
                                                                <input type="password" autoComplete="off" className="form-control" maxLength="16" placeholder="Password" name="password" onChange={this.handleUserInput} />
                                                                <FormErrors formErrors={this.state.formErrors} errorField={{ name: 'password' }} />
                                                            </div>
                                    ...
                                    <table >
                                        <tbody>
                                            <tr>
                                                ...
                                                <td className="price_cell text-center" style={{ paddingBottom: '2px' }}>
                                                    {(!this.packageData[this.state.selPackage.code].price) ? '' : Util.showPrice(this.packageData[this.state.selPackage.code].price, this.state.conversionRate, this.state.currency, this.state.currencyCode)}
                                                </td>
                                            </tr>
                                            <tr>
