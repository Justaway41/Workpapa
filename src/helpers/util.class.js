...
class Util {
  ...
  static getAddonData(geo, site) {
    ...
    addonData
      .filter(
        (element) =>
          site.pendingResume >= Globals.site.FAST_SERVICE_RESUME_LIMIT ||
          (element.name !== "group8" &&
            site.pendingResume <= Globals.site.FAST_SERVICE_RESUME_LIMIT)
      )
      .forEach((element) => {
        ...
        element.items.forEach((ele) => {
          ...
          ele.description = ele.description
            ? ele.description.replace(/##RESUMETEXT##/g, resumeText)
            : "";
          ele.description = ele.description
            ? ele.description.replace(/##RESUMEDAYS##/g, site.pendingResume)
            : "";
        });
  ...
  static getPackageData(geo) {
    ...
    Object.keys(packageData.service.product).forEach((element) => {
      packageData.service.product[element].name = packageData.service.product[
        element
      ].name.replace(/##RESUMETEXT##/g, resumeText);
      packageData.service.product[
        element
      ].description = packageData.service.product[element].description.replace(
        /##RESUMETEXT##/g,
        resumeText
      );
    });
